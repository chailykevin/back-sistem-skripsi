...
async function getPengajuanJudulRecord(conn, pengajuanJudulId) {
  const [[row]] = await conn.query(
    `SELECT pj.id, pj.npm, pj.status as pj_status,
            s.id as sidang_id, s.pengajuan_judul_id as sidang_pj_id,
     ...
     LEFT JOIN sidang s ON s.pengajuan_judul_id = pj.id
     LEFT JOIN revisi_pasca_sidang rps ON rps.pengajuan_judul_id = pj.id
     WHERE pj.id = ?
     LIMIT 1`,
    [pengajuanJudulId],
  );
...
async function generateSuratDoc(conn, pengumpulan, sidangRow, confirmations, sekprodiSig, namaSekprodi) {
  const [mhsRow] = await conn.query(
    `SELECT m.nama, m.npm, psk.no_hp
     FROM mahasiswa m
     LEFT JOIN pengajuan_sidang_kaprodi psk ON psk.pengajuan_judul_id = ?
     WHERE m.npm = ?
     LIMIT 1`,
    [pengumpulan.pengajuan_judul_id, pengumpulan.npm],
  );
  const mhs = mhsRow[0] ?? {};
  ...
  const [[psRow]] = await conn.query(
    `SELECT ps.nama as prodi_nama, ps.sekprodi_nidn
     ...
     WHERE s.pengajuan_judul_id = ?
     LIMIT 1`,
    [pengumpulan.pengajuan_judul_id],
  );

  const [[skripsiRow]] = await conn.query(
    `SELECT judul FROM skripsi WHERE pengajuan_judul_id = ? LIMIT 1`,
    [pengumpulan.pengajuan_judul_id],
  );
  ...
  const [sigMhs, sigPb1, sigPb2, sigPg1, sigPg2] = await Promise.all([
    ...
    getSigByNidn(conn, confMap["PEMBIMBING_1"]?.nidn ?? null),
    getSigByNidn(conn, confMap["PEMBIMBING_2"]?.nidn ?? null),
    getSigByNidn(conn, confMap["PENGUJI_1"]?.nidn ?? null),
    getSigByNidn(conn, confMap["PENGUJI_2"]?.nidn ?? null),
  ]);
...
// POST /pengumpulan-berkas-final/:pengajuanJudulId/init
exports.initPengumpulan = async (req, res, next) => {
  ...
  try {
    ...
    const [[pjRow]] = await conn.query(
      `SELECT id, npm FROM pengajuan_judul WHERE id = ? LIMIT 1`,
      [pengajuanJudulId],
    );
    if (!pjRow) {
    ...
    const [result] = await conn.query(
      `INSERT INTO pengumpulan_berkas_final (pengajuan_judul_id, npm, status) VALUES (?, ?, 'DRAFT')`,
...
// POST /pengumpulan-berkas-final/:pengajuanJudulId/submit
  ...
  try {
    ...
    const [[pengumpulan]] = await conn.query(
      `SELECT pbf.id, pbf.status, pbf.npm, s.pembimbing1_nidn, s.pembimbing2_nidn, s.penguji1_nidn, s.penguji2_nidn
       ...
       LEFT JOIN sidang s ON s.pengajuan_judul_id = pbf.pengajuan_judul_id
       WHERE pbf.pengajuan_judul_id = ? AND pbf.npm = ?
       LIMIT 1`,
      [pengajuanJudulId, npm],
    );
    if (!pengumpulan) {
    ...
    const confirmRows = [
      { role: "PERPUSTAKAAN", nidn: null, userId: perpustakaanUserId },
      { role: "LPPM", nidn: null, userId: lppmUserId },
      { role: "PEMBIMBING_1", nidn: pengumpulan.pembimbing1_nidn, userId: null },
      { role: "PEMBIMBING_2", nidn: pengumpulan.pembimbing2_nidn, userId: null },
      { role: "PENGUJI_1", nidn: pengumpulan.penguji1_nidn, userId: null },
      { role: "PENGUJI_2", nidn: pengumpulan.penguji2_nidn, userId: null },
    ];

    for (const row of confirmRows) {
      let resolvedUserId = row.userId;
      if (!resolvedUserId && row.nidn) {
        resolvedUserId = await getUserIdByNidn(conn, row.nidn);
      }
      await conn.query(
        `INSERT INTO pengumpulan_berkas_final_confirmations (pengumpulan_id, recipient_role, nidn, user_id)
         VALUES (?, ?, ?, ?)`,
        [pengumpulan.id, row.role, row.nidn ?? null, resolvedUserId ?? null],
      );
    }
    ...
    const link = `/pengumpulan-berkas-final/${pengajuanJudulId}`;
    for (const row of confirmRows) {
      let targetUserId = row.userId;
      if (!targetUserId && row.nidn) {
        targetUserId = await getUserIdByNidn(conn, row.nidn);
      }
      if (targetUserId) {
...
// POST /pengumpulan-berkas-final/:pengajuanJudulId/confirm
exports.confirmPengumpulan = async (req, res, next) => {
  ...
  try {
    ...
    const [[pengumpulan]] = await conn.query(
      `SELECT pbf.id, pbf.status, pbf.npm, pbf.pengajuan_judul_id,
              s.pembimbing1_nidn, s.pembimbing2_nidn, s.penguji1_nidn, s.penguji2_nidn,
              s.program_studi_id
       FROM pengumpulan_berkas_final pbf
       LEFT JOIN sidang s ON s.pengajuan_judul_id = pbf.pengajuan_judul_id
       WHERE pbf.pengajuan_judul_id = ?
       LIMIT 1`,
      [pengajuanJudulId],
    );
    if (!pengumpulan) {
    ...
    if (Number(confirmed) === Number(total)) {
      ...
      const allConfs = await conn.query(
        `SELECT recipient_role, nidn, user_id, confirmed_at
         FROM pengumpulan_berkas_final_confirmations
         WHERE pengumpulan_id = ?`,
        [pengumpulan.id],
      );
      ...
      if (pengumpulan.program_studi_id) {
        ...
        if (psRow?.sekprodi_nidn) {
          const sekprodiUserId = await getUserIdByNidn(conn, psRow.sekprodi_nidn);
          if (sekprodiUserId) {
            await insertNotification(
              conn,
              sekprodiUserId,
              ...
              `/pengumpulan-berkas-final/${pengajuanJudulId}`,
            );
          }
        }
      }
...
// POST /pengumpulan-berkas-final/:pengajuanJudulId/sign
...
// GET /pengumpulan-berkas-final/:pengajuanJudulId
exports.getPengumpulan = async (req, res, next) => {
  try {
    ...
    const [[pengumpulan]] = await db.query(
      `SELECT pbf.id, pbf.pengajuan_judul_id, pbf.npm, pbf.status, pbf.is_completed,
    ...
    const [confirmations] = await db.query(
      `SELECT id, recipient_role, nidn, user_id, confirmed_at
       FROM pengumpulan_berkas_final_confirmations
       WHERE pengumpulan_id = ?
    ...
    return res.json({
      ...
      data: {
        id: pengumpulan.id,
        pengajuanJudulId: pengumpulan.pengajuan_judul_id,
        npm: pengumpulan.npm,
...
// GET /pengumpulan-berkas-final/:pengajuanJudulId/files/:fileType
  ...
  try {
    ...
    const [rows] = await db.query(
      `SELECT pbf.id, pbf.pengajuan_judul_id, pbf.npm, pbf.status, pbf.is_completed,
       ...
       FROM pengumpulan_berkas_final pbf
       INNER JOIN sidang s ON s.pengajuan_judul_id = pbf.pengajuan_judul_id
       LEFT JOIN mahasiswa m ON m.npm = pbf.npm
       LEFT JOIN skripsi sk ON sk.pengajuan_judul_id = pbf.pengajuan_judul_id
      ...
      [nidn, nidn, nidn, nidn, nidn, nidn, nidn, nidn, nidn],
    );
    ...
    return res.json({
      ...
      data: filtered.map((r) => ({
        id: r.id,
        pengajuanJudulId: r.pengajuan_judul_id,
        npm: r.npm,
