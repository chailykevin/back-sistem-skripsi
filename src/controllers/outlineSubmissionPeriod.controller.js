...
// BELUM_DIBUKA (not yet open) / SEDANG_BUKA (open) / SUDAH_TUTUP (already closed)
function computeStatus(openAt, closeAt) {
  const now = Date.now();
  if (now < new Date(openAt).getTime()) return "BELUM_DIBUKA";
  if (now > new Date(closeAt).getTime()) return "SUDAH_TUTUP";
  return "SEDANG_BUKA";
}
...
function formatRow(r) {
  return {
    ...
    status: computeStatus(r.open_at, r.close_at),
...
// Checked against ALL periods regardless of tahun_akademik/periode_akademik so that at most one
// outline submission period can ever be open at the same time.
async function findOverlappingPeriod({ id, openAt, closeAt }) {
  const params = [closeAt, openAt];
...
exports.create = async (req, res, next) => {
  try {
    if (!req.user?.hasRole("ADMIN")) {
      return res.status(403).json({ ok: false, message: "Only admin can create outline submission periods" });
    }
    ...
    if (hasOverlap) {
      return res.status(409).json({
        ok: false,
        message: "Sudah ada periode lain dengan rentang tanggal yang tumpang tindih. Hanya boleh ada satu periode yang buka pada satu waktu.",
      });
    }
    ...
    const [rows] = await db.query(
      `SELECT id, tahun_akademik, periode_akademik, open_at, close_at, created_at, updated_at
       FROM outline_submission_period WHERE id = ?`,
      [result.insertId]
    );

    return res.status(201).json({ ok: true, message: "Outline submission period created", data: formatRow(rows[0]) });
  } catch (err) {
    return next(err);
  }
};
...
exports.update = async (req, res, next) => {
  try {
    if (!req.user?.hasRole("ADMIN")) {
      return res.status(403).json({ ok: false, message: "Only admin can update outline submission periods" });
    }
    ...
    const [existing] = await db.query(
      `SELECT id, open_at, close_at, tahun_akademik, periode_akademik FROM outline_submission_period WHERE id = ?`,
      [id]
    );
    if (!existing.length) {
      return res.status(404).json({ ok: false, message: "Outline submission period not found" });
    ...
    await db.query(`UPDATE outline_submission_period SET ${sets.join(", ")} WHERE id = ?`, params);
    ...
    return res.json({ ok: true, message: "Outline submission period updated", data: formatRow(rows[0]) });
  } catch (err) {
    return next(err);
  }
};
...
exports.remove = async (req, res, next) => {
  try {
    if (!req.user?.hasRole("ADMIN")) {
      return res.status(403).json({ ok: false, message: "Only admin can delete outline submission periods" });
    }
    ...
    const [existing] = await db.query(
      `SELECT id FROM outline_submission_period WHERE id = ?`,
      [id]
    );
    if (!existing.length) {
      return res.status(404).json({ ok: false, message: "Outline submission period not found" });
